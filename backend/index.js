...
app.post("/download", async (req, res) => {
    ...
    if (isInstagramReels) {
        ...
        const ytDlpArgs = ["-f", format, ...extraOptions, "-o", outputTemplate, url, "--yes-playlist", "--newline","--cookies", "backend\cookies.txt",];
        ...
        process.on("close", (code) => {
            ...
            const ytDlpArgs = ["--no-playlist", "-f", format, ...extraOptions, "-o", filePath, url, "--newline","--cookies", "backend\cookies.txt",];
            const downloadProcess = spawn(ytDlpPath, ytDlpArgs);
